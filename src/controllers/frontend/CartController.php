...
class CartController extends Controller
{
    ...
    public function actionProduct($id)
    {
        ...
        $cart = new CartForm();
        $model = ProductVariation::findOne((int)$id);
        return [
            'id' => $id,
            'name' => $model->product->title,
            'variant' => implode(', ', $model->parameterValues),
            'price' => $model->price_0,
            'category' => $model->product->categories ? $model->product->categories[0]->title : NULL,
        ];
    }
    ...
    public function actionCheckout()
    {
        ...
        if (!sizeof($model->cart->orderItems))
    ...
    public function actionPay($order_id)
    {
        ...
        if ($model->payment_type_id == PaymentType::ALFABANK) {
            ...
            $alfaRegistrator->execute();
            $this->redirect($alfaRegistrator->getFormUrl());
        }
